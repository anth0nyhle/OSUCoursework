...
for i = 1:n_c-1
    ...
    y_predc = y_c(i) + yp_intc * (h_c / 2); % predictor, compute y @ midpoint
    yp_midc = dydt(inv_c(i)+(h_c/2), y_predc); % use predictor y to predict slope @ midpoint
...
for i = 1:n_d-1
    k1 = dydt(inv_d(i), y_d(i)); % compute slope @ beginning of interval
    y_mid1 = y_d(i) + k1 * (h_d / 2); % compute y @ midpoint
    k2 = dydt(inv_d(i)+(h_d/2), y_mid1); % compute slope @ midpoint
    y_mid2 = y_d(i) + k2 * (h_d / 2); % compute y @ another midpoint
    k3 = dydt(inv_d(i)+(h_d/2), y_mid2); % compute slope @ another midpoint
    y_end = y_d(i) + k3 * h_d; % compute y @ end of interval
    k4 = dydt(inv_d(i+1), y_end); % compute slope @ end of interval
    phi = (1 / 6) * (k1 + (2 * k2) + (2 * k3) + k4); % compute average slope
    y_d(i+1) = y_d(i) + phi * h_d; % compute final y prediction
end
